from .schemas import (
    FuncionarioSchema, ContratoSchema, ContaBancariaSchema,
    AvaliacaoSchema, CargoSchema, DuracaoSchema, 
    EmpresaTerceirosSchema
)
from .models import (
    Funcionario, ContaBancaria, Contrato, 
    Duracao, Cargo, Avaliacao, 
    Empresa_Terceiros
)
...
class BaseModelRouter:
    ...
    def read(self):
        @self.router.get('/', response=List[self.schema], operation_id=f'{self.model.__name__.lower()}_listar')
        async def listagem(request):
            objetos = await sync_to_async(self.model.objects.all)()
            return list(objetos)

    def readid(self):
        @self.router.get('{nome}/', response=self.schema, operation_id=f'{self.model.__name__.lower()}_listar_id')
    ...
    def create(self):
        @self.router.post('/', response=self.schema, operation_id=f'{self.model.__name__.lower()}_criar')
    ...
    def update(self):
        @self.router.put('{nome}/', operation_id=f'{self.model.__name__.lower()}_atualizar')
    ...
    def delete(self):
        @self.router.delete('{nome}/delete', operation_id=f'{self.model.__name__.lower()}_deletar')
...
class SubModelRouter:
    def __init__(self, model, schema, tabela_pai, campo_tabela, cripto = None, decripto = None):
        self.model = model
        self.schema = schema
    ...
    def get_query(self, campo):
            obj_filho = self.model.objects.select_related(self.tabela_pai_nome).get(**{self.campo_tabela: campo})
            print(obj_filho)
            return obj_filho
    ...
    def readid(self):
        @self.router.get('{campo}/', response=self.schema, operation_id=f'{self.model.__name__.lower()}_listar_id')
        async def listagem_id(request, campo):   
            obj_filho = await sync_to_async(self.get_query)(campo)
            print(obj_filho)
            serialized_data = model_to_dict(obj_filho, fields=[f"{self.tabela_pai_nome}_id", *self.schema.__fields__.keys()])
            self.decryption(serialized_data)
            print(serialized_data)
            obj = {f"{self.tabela_pai_nome}_id": serialized_data[self.tabela_pai_nome], **serialized_data}
            return obj

    def create(self):
        @self.router.post('{campo}/create', operation_id=f'{self.model.__name__.lower()}_criar')
        async def create(request, data: self.schema, campo):
            obj_pai = await sync_to_async(get_object_or_404)(self.tabela_pai, **{self.campo_tabela: campo})
            data_dict = data.dict()
    ...
    def update(self):
        @self.router.put('{campo}/', operation_id=f'{self.model.__name__.lower()}_atualizar')
        async def update(request,  campo, data: self.schema):
            obj_pai = await sync_to_async(get_object_or_404)(self.tabela_pai, **{self.campo_tabela: campo})
            obj_filho = await sync_to_async(self.get_query)(campo)
            data_dict = data.dict()
            data_dict[f'{self.tabela_pai.__name__.lower()}'] = obj_pai
            if data_dict:
                self.encryption(data_dict)
    ...
    def delete(self):
        @self.router.delete('{campo}/delete', operation_id=f'{self.model.__name__.lower()}_deletar')
...
conta_bancaria_router = SubModelRouter(ContaBancaria, ContaBancariaSchema, Funcionario, 'id', cripto=cripto_banco.encrypt(), decripto=cripto_banco.decrypt())
...
empresa_terceiros_router = SubModelRouter(Empresa_Terceiros, EmpresaTerceirosSchema, Contrato, 'id')

# Nesting Routes
funcionario_router.router.add_router('{nome}/contaBancaria', conta_bancaria_router.router)
...
contrato_router.router.add_router('{id}/empreasTerceiros', empresa_terceiros_router.router)
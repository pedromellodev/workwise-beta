...
class Funcionario(BaseModel):
    ...
    escolaridade = models.CharField(
        max_length=3, choices=funci_escolaridade, default="EMC"
    )
    ...
    def idade(self):
        if self.dt_nasc:
            ...
            return (
                hoje.year
                - self.dt_nasc.year
                - ((hoje.month, hoje.day) < (self.dt_nasc.month, self.dt_nasc.day))
            )
        return None
...
class Cargo(BaseModel):
    contrato = models.OneToOneField(
        Contrato, verbose_name="contratos", on_delete=models.DO_NOTHING
    )
    cargos = models.CharField(
        max_length=3, choices=cargo_choices, verbose_name="cargos", default="SP"
    )
